...
export default function MainShowBox(props) {
  ...
  const showIds = [38913, 49964, 179, 21216];

  const getTvShow = async (id) => {
    const data = await getShow(id).then((res) => res.data);

    const imageData = await getImagesApi(id).then((res) => res.data);
    const backgroundData = imageData.filter(
      (obj) => obj?.type === "background"
    );

    const allShowObj = {
      ...data,
      resolutions: { ...backgroundData[1]?.resolutions },
    };
    setTvShow((tvShow) => [...tvShow, allShowObj]);
  };
  ...
  function getRandomShow() {
    return tvShow.map((show) => {
      return (
        <SwiperSlide key={`${show?.id}`} className="slide-show-box">
          <div className="show-box">
            <div className="image-holder">
              <img
                src={show?.resolutions?.original?.url || pictureComingSoon}
                alt=""
              />
